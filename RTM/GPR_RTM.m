function [R_profile,srcx,srcz,recx,recz] = GPR_RTM(ep,mu,sig,xprop,zprop,srcloc,recloc,t,npml,profile)
% RTM.m
%
% Syntax:  [R_profile,srcx,srcz,recx,recz] = GPR_RTM(ep,mu,sig,xprop,zprop,srcloc,recloc,t,npml,profile)
%
% where 
%       R_profile : output data of RTM
...
% ------------------------------------------------------------------------------------------------
    ...
    for it=numit:-1:1
        ...
        % add wave field to Ey
        for k=1:nsrc
               Ey(reci(k),recj(k))=profile(it,k);   % proflie
        end   
